...
class Filter extends React.Component {
  static propTypes = {
    fields: PropTypes.arrayOf(PropTypes.object), // 筛选项列表
  ...
  _renderFieldItem = field => {
    switch (field.type) {
      case FieldTypes.TEXT: {
        return <span>{field.content}</span>;
      }
  ...
  render() {
    ...
    return (
      <div className="search-form" id="listFilter" style={{ position: 'relative' }}>
        <Form className={styles.filter} onSubmit={this._onSubmit}>
          {fields.map(field => (
            <div key={field.key}>
              {field.label && (
                <span>
